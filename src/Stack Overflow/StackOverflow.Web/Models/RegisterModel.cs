...
namespace StackOverflow.Web.Models
{
	public class RegisterModel
	{
		...
		{
			var user = await _userManagerAdapter!.FindByEmailAsync(email);
			if (user == null)
				return null!;
			else
		...
		internal async Task EmailConfirmationTokenAsync()
        {
			var user = GetMember();
			await _userManagerAdapter!.EmailConfirmationTokenAsync(user);
		}
